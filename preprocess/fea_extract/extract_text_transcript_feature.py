...
def extract_bert_features(dataset_dir, model_id, output_file):
    class MyDataset(Dataset):
        ...
        def __getitem__(self, index):
            vid = self.vids[index]
            trans_row = self.text_df[self.text_df["vid"] == vid]
            if len(trans_row) == 0:
                # Handle missing transcript
                text = ""  # or skip this vid
                print(f"Warning: vid {vid} not found in transcript file.")
            else:
                text = trans_row["transcript"].values[0]
            return vid, text
...
extract_bert_features(
    "data/HateMM", "local_models/bert-base-uncased", "data/HateMM/fea/fea_transcript_bert-base-uncased.pt"
)
