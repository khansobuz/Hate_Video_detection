"""
Enhanced Continual Learning with CODA-Prompt (CVPR 2023)
Integrates state-of-the-art prompt-based CL into existing pipeline
"""
...
# ============================================================================
# DATASET AND COLLATOR (Keep same)
# ============================================================================

class HateMM_Novel_Dataset(HateMM_Dataset):
...
# ============================================================================
# ANTI-OVERFITTING COMPONENTS (Keep same)
# ============================================================================
...
# ============================================================================
# NEW: CODA-Prompt Components (CVPR 2023)
# ============================================================================

class PromptPool(nn.Module):
    """
    CODA-Prompt's key-query matching prompt pool
    Reference: Smith et al. "CODA-Prompt: COntinual Decomposed Attention-based Prompting" CVPR 2023
    """
    def __init__(self, pool_size=10, prompt_length=5, embed_dim=128, 
                 top_k=5, prompt_key_init='uniform'):
        super().__init__()
        ...
        # Prompt values
        self.prompt_values = nn.Parameter(torch.randn(pool_size, prompt_length, embed_dim))
        nn.init.uniform_(self.prompt_values, -1, 1)
        
        # Normalization
        self.key_norm = nn.LayerNorm(embed_dim)
        
    def forward(self, x_embed, train=True):
        """
        Args:
            x_embed: [B, D] - query features
            train: training mode flag
        Returns:
            prompts: [B, L, D] - selected prompts
            reduce_sim: similarity for regularization
        """
        B = x_embed.shape[0]
        
        # Normalize keys and queries
        prompt_keys_norm = F.normalize(self.prompt_keys, p=2, dim=1)
        x_embed_norm = F.normalize(x_embed, p=2, dim=1)
        
        # Calculate similarity [B, pool_size]
        similarity = torch.matmul(x_embed_norm, prompt_keys_norm.t())
        
        # Select top-k prompts
        if train:
            _, top_k_idx = torch.topk(similarity, self.top_k, dim=1)
            
            # Gather prompts [B, top_k, L, D]
            batched_prompts = self.prompt_values[top_k_idx]
            
            # Average over top-k
            prompts = batched_prompts.mean(dim=1)  # [B, L, D]
            
            # Orthogonal regularization term
            reduce_sim = (similarity ** 2).sum() / B
        else:
            # At test time, use weighted combination
            weights = F.softmax(similarity / 0.1, dim=1)  # [B, pool_size]
            prompts = torch.einsum('bp,pld->bld', weights, self.prompt_values)
            reduce_sim = 0.0
            
        return prompts, reduce_sim

class PromptGuidedAttention(nn.Module):
    """
    Attention mechanism guided by prompt pool
    """
    ...
    def forward(self, x, prompts):
        ...
        Args:
            ...
            prompts: [B, L, D] prompt embeddings
        """
        x = x.unsqueeze(1)  # [B, 1, D]
        
        # Prompt-guided attention
        attn_out, _ = self.attention(x, prompts, prompts)
...
class DynamicClassPrototypes(nn.Module):
    """
    Dynamic class prototypes with momentum update
    Stores evolving representations of each class
    """
    def __init__(self, num_classes=2, feat_dim=128, momentum=0.9):
        super().__init__()
    ...
    def update(self, features, labels):
        """
        Update prototypes with new samples using momentum
        """
        with torch.no_grad():
    ...
    def prototype_loss(self, features, labels):
        ...
        loss = F.cross_entropy(similarities / 0.07, labels)
        
        return loss

# ============================================================================
# STEP 3: PC-CML (Prompt-Calibrated Continual Memory Learning)
# Replaces original CMEL with CODA-Prompt integration
# ============================================================================

class PC_CML(nn.Module):
    """
    Prompt-Calibrated Continual Memory Learning
    Integrates CODA-Prompt with dynamic prototype memory
    """
    def __init__(self, feat_dim=128, num_classes=2, 
        ...
        # Prompt-guided attention
        self.prompt_attention = PromptGuidedAttention(feat_dim, num_heads=4, dropout=0.2)
        
        # Dynamic class prototypes
        self.class_prototypes = DynamicClassPrototypes(
            num_classes=num_classes,
            feat_dim=feat_dim,
            momentum=prototype_momentum
        )
        
        # Feature projection for prompt query
    ...
    def forward(self, features, labels=None, train=True):
        """
        Args:
            features: [B, D] input features
            labels: [B] ground truth labels (for prototype update)
            train: training mode
        Returns:
            enhanced_features: [B, D] prompt-enhanced features
            prompt_loss: regularization loss for prompts
        """
        # Generate query for prompt selection
        query_embed = self.query_proj(features)
        
        # Select prompts from pool
        prompts, reduce_sim = self.prompt_pool(query_embed, train=train)
        
        # Apply prompt-guided attention
        prompt_enhanced = self.prompt_attention(features, prompts)
        
        # Combine with original features
        enhanced_features = self.out_proj(prompt_enhanced + features)
        
        # Update prototypes during training
        if train and labels is not None:
            self.class_prototypes.update(enhanced_features.detach(), labels)
        
        # Compute losses
        prompt_reg_loss = reduce_sim if train else 0.0
        prototype_loss = self.class_prototypes.prototype_loss(enhanced_features, labels) if labels is not None else 0.0
        
        return {
            'features': enhanced_features,
            'prompt_reg_loss': prompt_reg_loss,
            'prototype_loss': prototype_loss,
            'prototypes': self.class_prototypes.get_prototypes()
        }

# ============================================================================
# STEP 4: UMAF (Keep same)
# ============================================================================
...
# ============================================================================
# MAIN MODEL WITH PC-CML
# ============================================================================

class NovelCLModel(nn.Module):
    """Enhanced with CODA-Prompt based PC-CML"""
    name = 'NovelCL_CODA_Prompt'
    def __init__(self, text_encoder, fea_dim=128, dropout=0.3, num_head=4, 
                 num_classes=2, num_epoch=100, label_smoothing=0.15, 
                 use_mixup=True, mixup_alpha=0.3, 
                 prompt_pool_size=10, prompt_length=5, prompt_top_k=5,
                 **kwargs):
        super().__init__()
        ...
        # Cross-modal attention (keep same)
        ...
        # Step 3: NEW PC-CML with CODA-Prompt
        self.pc_cml = PC_CML(
            feat_dim=fea_dim,
            num_classes=num_classes,
    ...
    def forward(self, **inputs):
        ...
        # Step 3: PC-CML with CODA-Prompt
        text_cml_out = self.pc_cml(text_graph_feat, labels, train=self.training)
        vision_cml_out = self.pc_cml(vision_graph_feat, labels, train=self.training)
        audio_cml_out = self.pc_cml(audio_graph_feat, labels, train=self.training)
        ...
        # Step 4: UMAF
        outputs = self.umaf(text_cml, vision_cml, audio_cml)
        
        # Add PC-CML specific outputs
        outputs['text_feat'] = text_cml
        outputs['vision_feat'] = vision_cml
        outputs['audio_feat'] = audio_cml
        outputs['combined_feat'] = (text_cml + vision_cml + audio_cml) / 3.0
        
        # Add CL losses
        outputs['prompt_reg_loss'] = (text_cml_out['prompt_reg_loss'] + 
                                      vision_cml_out['prompt_reg_loss'] + 
                                      audio_cml_out['prompt_reg_loss']) / 3.0
        outputs['prototype_loss'] = (text_cml_out['prototype_loss'] + 
                                     vision_cml_out['prototype_loss'] + 
                                     audio_cml_out['prototype_loss']) / 3.0
    ...
    def calculate_loss(self, **inputs):
        ...
        # PC-CML losses
        prompt_reg_loss = inputs.get('prompt_reg_loss', 0.0)
        prototype_loss = inputs.get('prototype_loss', 0.0)
        ...
        # Total loss with PC-CML components
        total_loss = (alpha * main_loss + 
                     (1 - alpha) * modality_loss + 
                     0.01 * prompt_reg_loss +  # Prompt diversity regularization
                     0.5 * prototype_loss)  # Prototype alignment
...
class Trainer:
    def __init__(self, cfg: DictConfig):
        ...
        self.collator = HateMM_Novel_Collator(**cfg.data)
        self.use_mixup = cfg.para.get('use_mixup', True)
        self.mixup_prob = 0.55
        
    def _reset(self, cfg, fold, type):
        train_dataset = HateMM_Novel_Dataset(task=cfg.task, fold=fold, split='train', **cfg.data)
        test_dataset = HateMM_Novel_Dataset(task=cfg.task, fold=fold, split='test', **cfg.data)
        if cfg.task == 'binary':
            valid_dataset = HateMM_Novel_Dataset(task=cfg.task, fold=fold, split='valid', **cfg.data)
    ...
    def run(self):
        ...
        for fold in self.dataset_range:
            ...
            logger.info(f'{Fore.CYAN}Current fold: {fold} - PC-CML with CODA-Prompt')
        ...
        logger.info(f'{Fore.MAGENTA}=== FINAL RESULTS (PC-CML with CODA-Prompt) ===')
    ...
    def _train(self, epoch: int):
        ...
        for batch in pbar:
            ...
            inputs['labels'] = labels  # PC-CML needs labels
            ...
            if use_mixup_batch:
                ...
                outputs.update({
                    ...
                    'prompt_reg_loss': outputs_pre.get('prompt_reg_loss', 0.0),
                    'prototype_loss': outputs_pre.get('prototype_loss', 0.0)
                })
    ...
    def _valid(self, split: str, epoch: int, use_earlystop=False, final=False):
        ...
        for batch in tqdm(dataloader, bar_format=f"{fcolor}{{l_bar}}{{bar}}{{r_bar}}"):
            ...
            inputs = {key: value.to(self.device) for key, value in batch.items()}
            labels = inputs.pop('labels')
            inputs['labels'] = labels  # PC-CML needs labels even in eval
...
def main(cfg: DictConfig):
    ...
    logger.info(f"{Fore.CYAN}Enhanced CL with CODA-Prompt (CVPR 2023)")
    logger.info(f"{Fore.CYAN}PC-CML: Prompt-Calibrated Continual Memory Learning")
    logger.info(f"{Fore.CYAN}{'='*80}")
