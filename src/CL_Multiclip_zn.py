...
"""
Complete Anti-Overfitting Novel Four-Step Method for MultiHateClip ZH (Chinese) Dataset
...
class MHClipZH_Dataset(Dataset):
    ...
    def _get_data(self, fold: int, split: str, task: str):
        data_path = Path('data/MultiHateClip/zh')
...
class MHClipZH_Novel_Dataset(MHClipZH_Dataset):
    def __init__(self, fold: int, split: str, task: str, num_pos=30, num_neg=90, **kwargs):
        super().__init__()
        fea_path = Path('data/MultiHateClip/zh/fea')  # CHANGED TO ZH
        sim_path = Path('data/MultiHateClip/zh/retrieval')  # CHANGED TO ZH
        
        self.data = self._get_data(fold, split, task)
        self.mfcc_fea = torch.load(fea_path / 'fea_audio_mfcc.pt', weights_only=True)
        
        # KEY CHANGE: Using Chinese BERT features
        self.text_fea = torch.load(fea_path / 'fea_text_all_bert-base-chinese.pt', weights_only=True)
    ...
    def __getitem__(self, idx):
        ...
        # Convert to float32 to ensure dtype consistency
        text_fea = self.text_fea[vid].float()
        vision_fea = self.frame_fea[vid].float()
        audio_fea = self.mfcc_fea[vid].float()
        ...
        # Convert all neighbor features to float32
        text_sim_pos_fea = torch.stack([self.text_fea[key].float() for key in all_sim_pos_vids])
        text_sim_neg_fea = torch.stack([self.text_fea[key].float() for key in all_sim_neg_vids])
        vision_sim_pos_fea = torch.stack([self.frame_fea[key].float() for key in all_sim_pos_vids])
        vision_sim_neg_fea = torch.stack([self.frame_fea[key].float() for key in all_sim_neg_vids])
        audio_sim_pos_fea = torch.stack([self.mfcc_fea[key].float() for key in all_sim_pos_vids])
        audio_sim_neg_fea = torch.stack([self.mfcc_fea[key].float() for key in all_sim_neg_vids])
...
# ============================================================================
# MODEL: All architecture components are IDENTICAL to EN version
# Just change the model name to indicate it's for Chinese dataset
# ============================================================================

# Copy all classes from EN version:
# - DropPath
# - BidirectionalCrossModalAttention  
# - EnhancedGraphAttention
# - FAME
# - GraphAttentionLayer
# - R3GC
# - MemoryBank
# - CMEL
# - UMAF
# - NovelCLModel (change name to NovelCL_AntiOverfit_MHClipZH)
# - BinaryClassificationMetric
# - EarlyStopping
# - Trainer (update to use MHClipZH_Novel_Dataset and MHClipZH_Novel_Collator)

# The only changes needed:
# 1. Dataset class: MHClipZH_Novel_Dataset
# 2. Collator class: MHClipZH_Novel_Collator
# 3. Text feature file: fea_text_all_bert-base-chinese.pt
# 4. Data path: data/MultiHateClip/zh

# All training logic, model architecture, loss functions, and regularization
# techniques remain EXACTLY the same!

"""
SUMMARY OF CHANGES FROM EN TO ZH VERSION:
==========================================
1. Data path: 'data/MultiHateClip/en' → 'data/MultiHateClip/zh'
2. Text features: 'fea_title_trans_bert-base-uncased.pt' → 'fea_text_all_bert-base-chinese.pt'
3. Class names: MHClipEN → MHClipZH
4. Model name: NovelCL_AntiOverfit_MHClipEN → NovelCL_AntiOverfit_MHClipZH

Everything else (model architecture, training, regularization) is IDENTICAL!
"""

# [INCLUDE ALL OTHER CLASSES FROM ORIGINAL CODE: DropPath, BidirectionalCrossModalAttention, 
# EnhancedGraphAttention, FAME, R3GC, GraphAttentionLayer, MemoryBank, CMEL, UMAF]

# Due to length constraints, I'm providing the key differences. The model architecture 
# and training code remain the same. Here are the main changes:

# 1. Dataset path changed from 'data/HateMM' to 'data/MultiHateClip/en'
# 2. Feature file changed from 'fea_transcript_bert-base-uncased.pt' to 'fea_title_trans_bert-base-uncased.pt'
# 3. All other model components (FAME, R³GC, CMEL, UMAF) remain identical

# Copy all model classes (DropPath through UMAF) from the original code exactly as-is

# The full training code, metrics, and trainer class also remain the same.
# Only the dataset loading paths need to be updated as shown above.

# For the complete code, combine:
# 1. The dataset classes above (MHClipEN_Novel_Dataset, MHClipEN_Novel_Collator)
# 2. All model architecture classes from your original code (unchanged)
# 3. The trainer and main function (unchanged)

# This gives you a working implementation for MultiHateClip EN dataset!
"""
...
class Trainer:
    def __init__(self, cfg: DictConfig):
        ...
        self.collator = MHClipZH_Novel_Collator(**cfg.data)
    ...
    def _reset(self, cfg, fold, type):
        train_dataset = MHClipZH_Novel_Dataset(task=cfg.task, fold=fold, split='train', **cfg.data)
        test_dataset = MHClipZH_Novel_Dataset(task=cfg.task, fold=fold, split='test', **cfg.data)
        if cfg.task == 'binary':
            valid_dataset = MHClipZH_Novel_Dataset(task=cfg.task, fold=fold, split='valid', **cfg.data)
...
@hydra.main(version_base=None, config_path="config", config_name="config_en")
