...
from utils.core_utils import set_worker_seed
...
class Trainer():
    ...
    def _reset(self, cfg, fold, type):
        train_dataset = get_dataset(cfg.model, cfg.dataset,task=cfg.task, fold=fold, split='train', **cfg.data)
        if hasattr(cfg, 'general') and cfg.general:
            logger.info(f"Using {cfg.general.dataset} as test dataset!")
            test_dataset = get_dataset(cfg.model, cfg.general.dataset,task=cfg.task, fold=fold, split='test', **cfg.data)
        else:
            test_dataset = get_dataset(cfg.model, cfg.dataset,task=cfg.task, fold=fold, split='test', **cfg.data)
        if cfg.task == 'binary':
            valid_dataset = get_dataset(cfg.model, cfg.dataset, task=cfg.task,fold=fold, split='valid', **cfg.data)
        self.train_dataloader = DataLoader(train_dataset, batch_size=cfg.batch_size, collate_fn=self.collator, num_workers=min(32, cfg.batch_size//2), shuffle=True, generator=self.generator, worker_init_fn=partial(worker_init, seed=cfg.seed))
        self.test_dataloader = DataLoader(test_dataset, batch_size=cfg.batch_size, collate_fn=self.collator, num_workers=min(32, cfg.batch_size//2), shuffle=False, generator=self.generator, worker_init_fn=partial(worker_init, seed=cfg.seed))
        if cfg.task == 'binary': self.valid_dataloader = DataLoader(valid_dataset, batch_size=cfg.batch_size, collate_fn=self.collator, num_workers=min(32, cfg.batch_size//2), shuffle=False, generator=self.generator, worker_init_fn=partial(worker_init, seed=cfg.seed))
